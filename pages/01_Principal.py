...
# =============================================================================
    ...
    with tab_mensagens:
        ...
        st.markdown(f"""
            <style>
                ...
                div[data-testid="stForm"] textarea, div[data-testid="stForm"] input {{ font-size: 1.4rem !important; font-weight: bold !important; border: 3px solid var(--cor-texto) !important; }}
            </style>
        """, unsafe_allow_html=True)
    ...
    <div style="display: flex; gap: 15px; margin-bottom: 30px;">
        <div style="flex: 1; background: var(--cor-branco); border: 4px solid var(--cor-texto); padding: 20px; text-align: center; box-shadow: 6px 6px 0px var(--cor-texto);">
            <div style="font-family: 'Archivo Black'; font-size: 0.9rem; color: #666;">AÇÕES NO PERÍODO</div>
            <div style="font-family: 'Archivo Black'; font-size: 3rem; color: var(--cor-texto); line-height: 1; margin-top: 10px;">{total_acoes}</div>
        </div>
        <div style="flex: 1; background: var(--cor-branco); border: 4px solid var(--cor-texto); padding: 20px; text-align: center; box-shadow: 6px 6px 0px var(--cor-texto);">
            <div style="font-family: 'Archivo Black'; font-size: 0.9rem; color: #666;">MEMBROS ATIVOS</div>
            <div style="font-family: 'Archivo Black'; font-size: 3rem; color: var(--cor-secundaria); line-height: 1; margin-top: 10px;">{ativos_unicos}</div>
        </div>
        <div style="flex: 1; background: var(--cor-branco); border: 4px solid var(--cor-texto); padding: 20px; text-align: center; box-shadow: 6px 6px 0px var(--cor-texto);">
            <div style="font-family: 'Archivo Black'; font-size: 0.9rem; color: #666;">PRESENÇA FÍSICA</div>
            <div style="font-family: 'Archivo Black'; font-size: 3rem; color: var(--cor-texto); line-height: 1; margin-top: 10px;">{checkins_total}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
        ...
        if not df_f.empty:
            ...
            st.dataframe(df_rank.sort_values(by='Total de Ações', ascending=False), width='stretch',
                         hide_index=True)
    ...
    with tab_mapa:
        ...
        if not df_geo.empty:
            ...
            for _, row in df_geo.iterrows():
                popup_content = f"""
                <div style="font-family: sans-serif; min-width: 180px; border: 2px solid var(--cor-texto); padding: 10px; background-color: var(--cor-branco);">
                    <b style="color: var(--cor-secundaria); text-transform: uppercase; font-size: 14px;">{row['Nome']}</b><br>
...
# =============================================================================
    ...
    with tab_sistema:
        ...
        with c1:
            st.markdown(f"""
                <div style='background-color: var(--cor-fundo); border: 3px solid var(--cor-texto); 
                            padding: 20px; box-shadow: 4px 4px 0px var(--cor-texto);'>
                    <h3 style='margin-top:0; color: var(--cor-texto);'>📦 PACOTES INSTALADOS</h3>
            """, unsafe_allow_html=True)
        ...
        with c2:
            st.markdown(f"""
                <div style='background-color: var(--cor-fundo); border: 3px solid var(--cor-texto); 
                            padding: 20px; box-shadow: 4px 4px 0px var(--cor-texto);'>
                    <h3 style='margin-top:0; color: var(--cor-texto);'>📊 ESTATÍSTICAS DE USO</h3>
            """, unsafe_allow_html=True)
